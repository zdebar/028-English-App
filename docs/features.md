...
| --- | --- | --- | --- | --- |
| App shell | Initializes the app and renders global UI around routes. | `frontend/src/App.tsx`, `Header`, `Footer` | `useAuthStore`, `useToastStore`, `usePeriodicSync`, `useUserStoreSync`, `useAudioLoader`, `useThemeLoader`, `useDailyStatsReset` | Mounting `App` starts auth, sync, stats, audio, theme, toasts, overlay, and analytics. |
...
| Google sign-in | Sign in or convert to a durable account. | `GoogleAuthButton`, `ConvertAnonymousUserButton`, `IdentityLinkConflictModal` | Supabase Auth provider flow, one-time `sessionStorage` guest fallback | Conversion UI is shown from Home for anonymous users. If the Google identity already belongs to another account, the guest can stay anonymous or explicitly sign in without transferring progress; a failed/cancelled sign-in restores the guest session. |
...
| Grammar review | Review previously started grammar items whose schedule is due. | `/practice/grammar`, `Practice mode="grammar"`, `usePracticeDeck` | `UserBlock.getReadyGrammarPracticeState`, `UserItem.getPracticeDeck` | Shares save, score, audio, and unload recovery behavior with vocabulary review. |
| New grammar | Learn the next unlocked grammar block in staged rounds. | `/practice/new-grammar`, `NewGrammarPractice`, `useNewGrammarPracticeDeck` | `UserBlock`, `UserItem`, optional `Grammar` detail | Home unlock checks expose availability; completion marks block mastered and saves item progress. |
| Progress actions | Mark an item as hint/repeat/known/skip/mastered. | `HintButton`, `RepeatButton`, `KnownButton`, `MasterItemButton` | `UserItem` progress fields, `UserScore` daily count | Progress writes dispatch dashboard refresh events and are pushed by later sync. |
...
| Periodic sync | Push local changes and pull remote updates. | `usePeriodicSync`, `dataSync` | Supabase PostgreSQL, sync metadata, `useSyncStore` | Successful sync sets flags, increments `syncRevision`, refreshes dashboard stats, and syncs audio archives. |
...
| Sync store | Expose sync status and successful-sync revision. | `useSyncStore` | In-memory flags and `syncRevision` | Home reads `isSyncError`; `HomePracticeButtons` reads `syncRevision` to reload readiness after sync. |
| User stats store | Cache dashboard-level stats. | `useUserStore`, `useUserStoreSync` | IndexedDB model queries | Listens for `levelsUpdated` and `dailyCountUpdated`. |
