...
| Navigation | Provides Home/profile/theme navigation and footer content. | `Header`, `Footer`, route buttons | Router state, auth state, theme store | Footer renders only on the exact Home and Profile routes; route buttons use `ROUTES` constants. |
...
| Initial block training | Learn the lowest ordered unstarted practice block in staged rounds. | `/practice/block-training`, `BlockTrainingPractice`, `useBlockTrainingDeck` | `UserBlock`, `UserItem`, `PracticeSession` | Empty blocks are ignored; completion starts the block and returns Home to recalculate the next block. |
