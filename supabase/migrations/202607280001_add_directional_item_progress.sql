...
ALTER TABLE public.user_items
  ADD COLUMN progress_cz_to_en INTEGER NOT NULL DEFAULT 0
    CHECK (progress_cz_to_en >= 0),
  ADD COLUMN progress_en_to_cz INTEGER NOT NULL DEFAULT 0
    CHECK (progress_en_to_cz >= 0),
  ADD COLUMN next_at_cz_to_en TIMESTAMPTZ,
  ADD COLUMN next_at_en_to_cz TIMESTAMPTZ,
  ADD COLUMN mastered_at_cz_to_en TIMESTAMPTZ,
  ADD COLUMN mastered_at_en_to_cz TIMESTAMPTZ;

UPDATE public.user_items
SET
  progress_cz_to_en = FLOOR(progress / 2.0)::INTEGER,
  progress_en_to_cz = FLOOR(progress / 2.0)::INTEGER,
  next_at_cz_to_en = next_at,
  next_at_en_to_cz = next_at,
  mastered_at_cz_to_en = CASE
    WHEN FLOOR(progress / 2.0)::INTEGER >= 12 THEN mastered_at
    ELSE NULL
  END,
  mastered_at_en_to_cz = CASE
    WHEN FLOOR(progress / 2.0)::INTEGER >= 12 THEN mastered_at
    ELSE NULL
  END;
...
ALTER TABLE public.user_items_history
  DROP CONSTRAINT user_items_history_pkey,
  ADD COLUMN direction TEXT NOT NULL DEFAULT 'legacy' -- NOSONAR: shared SQL/PLpgSQL constants are not available across these scopes
    CHECK (direction IN ('czToEn', 'enToCz', 'legacy')),
  ADD PRIMARY KEY (user_id, item_id, created_at, direction);
