...
ALTER TABLE public.lessons
  DROP CONSTRAINT IF EXISTS lessons_sort_order_key;
ALTER TABLE public.blocks
  DROP CONSTRAINT IF EXISTS blocks_sort_order_key;
ALTER TABLE public.items
  DROP CONSTRAINT IF EXISTS items_sort_order_key;

WITH ordered AS (
  SELECT id, ROW_NUMBER() OVER (PARTITION BY level_id ORDER BY sort_order, id)::INTEGER AS position
  FROM public.lessons
)
...
WITH ordered AS (
  SELECT id, ROW_NUMBER() OVER (PARTITION BY lesson_id ORDER BY sort_order, id)::INTEGER AS position
  FROM public.blocks
)
UPDATE public.blocks AS target
SET sort_order = ordered.position
FROM ordered
WHERE target.id = ordered.id;

WITH ordered AS (
  SELECT id, ROW_NUMBER() OVER (PARTITION BY block_id ORDER BY sort_order, id)::INTEGER AS position
  FROM public.items
)
...
ALTER TABLE public.lessons
  ADD CONSTRAINT lessons_level_sort_order_key
  UNIQUE (level_id, sort_order) DEFERRABLE INITIALLY DEFERRED;
ALTER TABLE public.blocks
  ADD CONSTRAINT blocks_lesson_sort_order_key
