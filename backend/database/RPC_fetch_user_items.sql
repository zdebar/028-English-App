...
BEGIN
  ...
  LEFT JOIN public.levels levels
    ON lessons.level_id = levels.id
  WHERE ui.updated_at > last_synced_at
    OR i.updated_at > last_synced_at 
    OR levels.updated_at > last_synced_at
    OR lessons.updated_at > last_synced_at 
